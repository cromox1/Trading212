...
class ReadAllDataText(FxReadDataText_ToolTip):
    ...
    def main_collect_data(self, currency, value_EMA, time_period, grph_div_start, dict_fx):
        ...
        # print('NEAREMA = ', str("%.8f" % round(nearema1, 8)) , str("%.8f" % round(nearema2, 8)))
        if gradient3x <= gradient2x <= gradient1x and abs(gradient1x) >= float(0.7) and \
                float(datalist[-1]) >= float(datalist[-2]) and \
                (nearema1 < float(0.00015) or float(datalist[-1]) < float(emalist[-1])):
            text1 = text1 + ' BUY/LONG1 @' + str(datalist[-2]) + '<' + str(datalist[-1])
            tindakan[currency] = tindakan[currency] + (1*markah)
        if gradient3x >= gradient2x >= gradient1x and abs(gradient1x) >= float(0.7) and \
                float(datalist[-1]) <= float(datalist[-2]) and \
                (nearema1 > float(0.00055) and float(datalist[-1]) > float(emalist[-1])):
        ...
        if float(datalist[-5]) < float(emalist[-5]) and float(datalist[-2]) >= float(emalist[-2]) \
                and float(datalist[-1]) >= float(datalist[-2]) and nearema1 < float(0.00015):
            text1 = text1 + ' BUY/LONG3 @' + str(datalist[-2]) + '<' + str(datalist[-1])
            tindakan[currency] = tindakan[currency] + (1*markah)
        if float(datalist[-5]) > float(emalist[-5]) + float(0.00055) and float(datalist[-1]) <= float(datalist[-2]) \
                and ((float(datalist[-2]) >= float(emalist[-2]) and nearema1 > float(0.00025)) or
                     (float(datalist[-2]) <= float(emalist[-2]) and nearema1 < float(0.00015))):

...
dataexcel = pd.read_excel('tmpdir/' + tmpfilename, sheet_name='New Issues and IPOs', skiprows=range(6), usecols=['Company', 'Date', 'Issue Price', 'Currency'])
...
for kk,vv in dict_start_sort.items():
    if type(kk) == str and vv.year > 2017:
        if kk in dict_price.keys():
            print(num, ')', vv, ' // ', kk, ' // ', dict_currency[kk], dict_price[kk])
        else:
            print(num, ')', vv, ' // ', kk, ' //  GBP 0.0 #')
        num += 1

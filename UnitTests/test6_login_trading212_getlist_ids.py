...
# valuetofind = '//*[contains(@id, "chart-view")]'
...
if len(list_ids) >= 1:
    ...
    for idx in list_ids:
        try:
            # print(i, idx.get_attribute('id'))
            idxx = idx.get_attribute('id')
            idzz = idxx.replace('-chart-view', '')
            # print(i, idxx, idzz, end='')
            print(i, idxx, end='')
            try:
                if 'GBP/USD' in driver.find_element_by_id(idzz).text:
                    idx.click()
                    print(' / CLICKABLE')
                    break
                else:
